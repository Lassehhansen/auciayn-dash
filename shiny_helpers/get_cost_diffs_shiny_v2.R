...
all_expected_costs <- cost_grid_with_models %>%
  rowwise() %>%
  mutate(expected_costs = list(get_expected_costs(cur_data(), n = 100))) %>%
  unnest(expected_costs)

library(furrr)
library(dplyr)
library(future)

plan(multisession)  # Choose the number of workers based on your machine's capabilities


# Convert the data frame to a list of row tibbles
rows <- pmap(list(cost_grid_with_models), as.list)

# Use future_pmap to apply the function in parallel, ensuring we pass rows correctly
results_list <- future_pmap(rows, function(row) {
  # Convert the list back to a tibble
  row_df <- tibble::tibble(!!!row)
  get_expected_costs(row_df, n = 100)
})

# Combine the results into a single data frame
all_expected_costs <- bind_rows(results_list)


# Combine the results into a single dataframe
all_expected_costs <- bind_rows(results_list)

all_expected_costs$cost_factor = as.factor(paste0("Cost FN: ", all_expected_costs$cost_fn, " Cost FP: ", all_expected_costs$cost_fp))


write_csv(all_expected_costs, "simulation_data/thresholds_exp_costs_shiny.csv")
