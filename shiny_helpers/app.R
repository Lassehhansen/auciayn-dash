...
#setwd("shiny")
#source("calculate_auec.R")
#source("shiny_helper.R")
source("shiny_helper_v2.R")
# Define UI
ui <- fluidPage(
  tags$head(
    tags$style(HTML("
  ...
  fluidRow(
    column(width = 3,  # Adjust the width as necessary
           wellPanel(
             h3("Model Control Panel", style = "margin-top: 0;"),
           #sliderInput("cost_fn", "Cost of False Negative's", min = 0, max = 8, value = 4, step = 2, width = '100%'),
           #sliderInput("cost_fp", "Cost of False Positive's", min = 0, max = 8, value = 4, step = 2, width = '100%'),
           ...
           selectInput("p_pos_space", "How Many Positive Cases?",
            ...
            selectInput("ev_pos_a", "Minority Group Disease Presence",
                                   choices = c("63%" = 0.63, "72%" = 0.72, "80%" = 0.80), width = '100%'),
    ...
    column(width = 9,
           wellPanel(
             h3("Model Choice", style = "margin-top: 0;"),
...
server <- function(input, output) {
  ...
  cost_thresh_data <- reactive({
    cost_thresh_data <- read_csv("simulation_data/thresholds_exp_costs_shiny_v2.csv")
  ...
  auec_diff_data <- reactive({
    auec_diffs <- read_csv("simulation_data/auec_data_shiny_v3.csv")
  ...
  freq_data <- reactive({
    req(cost_thresh_data()) 
    cost_thresh_data <- cost_thresh_data()
    
    cost_thresh_data_tot <- cost_thresh_data$cost_thresh_data_tot
    
    cost_thresh_data_tot_auroc = cost_thresh_data_tot %>% filter(model_choice == "AUROC")
    cost_thresh_data_tot_auprc = cost_thresh_data_tot %>% filter(model_choice == "AUPRC")

    cost_thresh_data_auroc = sample_population(1000,
                                               cost_thresh_data_tot_auroc$attribute_ratio,
                                               cost_thresh_data_tot_auroc$alpha_pos_a,
                                               cost_thresh_data_tot_auroc$beta_pos_a,
                                               cost_thresh_data_tot_auroc$alpha_neg_a,
                                               cost_thresh_data_tot_auroc$beta_neg_a,
                                               cost_thresh_data_tot_auroc$alpha_pos_b,
                                               cost_thresh_data_tot_auroc$beta_pos_b,
                                               cost_thresh_data_tot_auroc$alpha_neg_b,
                                               cost_thresh_data_tot_auroc$beta_neg_b,
                                               cost_thresh_data_tot_auroc$p_pos)
    
    cost_thresh_data_auprc = sample_population(1000,
                                               cost_thresh_data_tot_auprc$attribute_ratio,
                                               cost_thresh_data_tot_auprc$alpha_pos_a,
                                               cost_thresh_data_tot_auprc$beta_pos_a,
                                               cost_thresh_data_tot_auprc$alpha_neg_a,
                                               cost_thresh_data_tot_auprc$beta_neg_a,
                                               cost_thresh_data_tot_auprc$alpha_pos_b,
                                               cost_thresh_data_tot_auprc$beta_pos_b,
                                               cost_thresh_data_tot_auprc$alpha_neg_b,
                                               cost_thresh_data_tot_auprc$beta_neg_b,
                                               cost_thresh_data_tot_auprc$p_pos)
    
    return(list(cost_thresh_data_auroc = cost_thresh_data_auroc, cost_thresh_data_auprc = cost_thresh_data_auprc))
  })
  ...
  output$plot_freq_tot <- renderPlot({
    ...
    freq_data <- freq_data()
    freq_data_auroc = freq_data$cost_thresh_data_auroc
    freq_data_auprc = freq_data$cost_thresh_data_auprc
    
    freq_data_auroc$model_choice = "AUROC"
    freq_data_auprc$model_choice = "AUPRC"
    
    freq_data_tot = rbind(freq_data_auroc, freq_data_auprc)
